...
> **代码位置**：`backend/conch/api/`（7 文件）、`backend/conch/runtime/sandbox/docker_sandbox.py`
...
async def stream_chat(session_id, req: ChatRequest):
    ...
    # 构建 LangGraph 图（注入 tools + system_prompt）
  ...
  └─ POST /api/chat/sessions/{id}/stream
       ...
       └─ orchestrator.run(task, [], state)
            └─ LangGraphHookBridge → 语义 Hook
                 ...
                 └─ hook_bus.fire("pre_tool") → MCP execute → yield tool_result
...
- WebSocket HITL → 阶段二 `api/websocket.py`
