...
SessionDB 新增按更新时间倒序的受限会话列表；会话消息通过 RBAC API 暴露给 Web 工作台。HookExecutor 幂等创建 `hook_executions` 表与索引，保存状态、退出码、截断输出和耗时。长工具输出不进入 SQLite，落入 storage 下权限 `0600` 的独立制品，数据库与上下文只保留引用。
