...
**90%** — P2 设计交付物基本全部实现，Docker 沙箱代码完整但未在 Windows 环境实跑验证。
...
| Docker 沙箱 | Docker 后端 + hard_reset | ✅ 已完成 | 90% (未实跑) |
...
|------|---------|---------|----------|
| E 层 | Local 沙箱 ✅ | + Docker 沙箱后端 + hard_reset + snapshot/restore | 90% (Docker 未实跑) |
...
### 偏差 1：SummaryArchive 的 LLM 调用为可选

- **偏差点**：设计文档要求摘要归档调用 auxiliary model，实际实现为可选 (llm_caller=None 时跳过)
- **原因**：P2 阶段未实现 auxiliary model 的独立配置，使用主模型做摘要成本高
- **影响**：当 llm_caller=None 时，前两步（清理+折叠）仍执行，但不会做 LLM 摘要
- **后续**：P3 接入 Reviewer 后自然解决

### 偏差 2：Prompt Caching 仅 Anthropic 生效
...
- **偏差点**：设计文档要求 Docker 后端，代码已实现但未在 Windows 环境实际运行
- **原因**：Windows 开发环境未安装 Docker Desktop
- **影响**：Docker 相关代码逻辑完整但可能有路径/权限问题
- **后续**：在 Linux/Docker 环境中验证

### 偏差 4：MetaMemory FTS5 降级处理
...
|---------|---------|----------|---------|------|------|
| 沙箱 (E层) | test_sandbox.py | 20 | 0 | 20 | 20 |
| 状态存储 (S层) | test_state.py | 16 | 0 | 16 | 16 |
| 核心工具 (T层) | test_tools.py | 17 | 0 | 17 | 17 |
| 工具系统 (T层) | test_tool_system.py | 20 | 0 | 20 | 20 |
| 引擎 (L层) | test_engine.py | 20 | 0 | 20 | 20 |
| 集成测试 | test_integration.py | 5 | 0 | 5 | 5 |
| C 层 (P2 新增) | test_context.py | 0 | 32 | 32 | 32 |
| P2 综合 (P2 新增) | test_p2.py | 0 | 32 | 32 | 32 |
| **总计** | **8 个文件** | **98** | **64** | **162** | **162** |
...
| - | -------- | -------- | -------- | ------------ |
| 1 | SummaryArchive LLM 调用未接入主模型 | 压缩第三步不执行 | 前两步足够应对大部分场景 | P3 |
| 2 | Docker 沙箱未实跑验证 | Docker 代码可能有路径问题 | 使用 Local 沙箱 | P3 (Linux 环境) |
| 3 | MCP 客户端未实现 | 不能接入 MCP 工具生态 | 使用核心工具 | P3 |

### 中优先级

| # | 问题描述 | 影响范围 | 临时规避 | 建议修复阶段 |
| - | -------- | -------- | -------- | ------------ |
| 4 | Context Engine 未实现 auto-compact 自动触发 | 需手动调用 compact | maintain() 已占位 | P3 |
| 5 | 记忆提取 LLM 模式未接入主模型 | 仅规则提取 | 规则提取覆盖常见模式 | P3 |
| 6 | Docker snapshot/restore 未用 commit | hard_reset 直接重建 | 功能正常 | P3 |
| 7 | Coordinator 多 Agent 编排未实现 | Subagent 有管理无编排 | 手动 spawn | P4 |

### 低优先级

| # | 问题描述 | 影响范围 | 临时规避 | 建议修复阶段 |
| - | -------- | -------- | -------- | ------------ |
| 8 | FTS5 降级为 LIKE | 元记忆搜索性能略差 | 功能正常 | P3 |
| 9 | Skill Curator 自改进未实现 | Skill 不自动优化 | 手动管理 | P4 |
| 10 | 持久记忆向量检索未实现 | LongTermMemory 用 LIKE | 数据量小时足够 | P3/P4 |
...
2. **ContextCompressor 三步管线**：P3 接入 Reviewer 后 SummaryArchive 自动启用
...
3. **Context Engine auto-compact**：在 maintain() 中检查 token_count vs budget，自动触发 compact
4. **SummaryArchive LLM 接入**：将主模型传入 ContextCompressor 的 llm_caller
5. **React Web Console**：新建 apps/web/ 目录，Vite + React + TypeScript
