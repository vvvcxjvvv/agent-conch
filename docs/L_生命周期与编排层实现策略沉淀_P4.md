...
HookExecutor 已作为 Layer 接入 graph_start/node_start/node_end/event/graph_end 生命周期；Hook 按配置稳定串行执行，fail-closed 可中止图或阻断节点，结果写入 SQLite 并通过管理 API 查询。MCP 连接在首次运行前初始化，在 API/Engine 关闭时统一释放，避免 stdio 子进程泄漏。
